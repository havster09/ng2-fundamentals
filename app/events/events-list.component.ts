...
@Component({
    ...
    template: `
        <div>
            ...
            <hr/>
            <div class="panel" *ngFor="let player of bucksPlayers">
                <player-thumbnail
                        [player]="player"
                        (reset)="handleReset($event)"
                        (resetAll)="handleResetAll()"
                        (shoot)="handleShoot($event)"></player-thumbnail>
            </div>
...
export class EventsListComponent {
    ...
    bucksPlayers = [
        {name: 'giannis', points: 0},
        {name: 'greg', points: 0}
    ];

    handleShoot(score) {
        this.bucksPlayers.filter((player) => player.name === score.name)
            .map(player => {
                player.points += score.points;
            });
    }

    handleReset(playerName) {
        this.bucksPlayers.filter(player => player.name === playerName)
            .map(player => {
                player.points = 0;
            });
    }
